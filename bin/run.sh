...
# ===== 必须的环境变�?=====
: "${ETDATA_TOKEN:?未提�?ETDATA_TOKEN，已终止}"
: "${ETDATA_SCRIPT:?未提�?ETDATA_SCRIPT（脚本名），已终止}"
...
if ! curl -fsSL "$ENC_FILE_URL" -o "$TMP_ENC"; then
  echo "错误：无法下载脚本文�?(HTTP 404)"
  echo "请检查：1. 脚本名是否正�?2. 仓库中是否已存在 .enc 加密文件"
  exit 1
fi
...
# 尝试使用 Token 解密
set +e # 暂时允许失败以便捕获错误
# 为了兼容 CentOS 7 (OpenSSL 1.0.2)，移�?-pbkdf2 �?-iter，并显式指定 -md sha256
...
if ! head -n 1 "$TMP_SCRIPT" | grep -q "^#"; then
  echo "错误：解密后的文件格式异常，可能�?Token 不匹配�?
  exit 1
fi
